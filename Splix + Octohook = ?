...
do
    ...
    function library:New(info)
        ...
        function window:Watermark(info)
            ...
            --
            local info = info or {}
            local watermark_name = info.name or info.Name or info.title or info.Title or string.format("nomercy.rip || uid : %u || ping : %u || fps : %u", 1, 100, 200)
            --
            ...
            --
            watermark_title.Text = string.format("nomercy.rip|| uid : %u || ping : %i || fps : %u", 1, tostring(library.shared.ping), library.shared.fps)
        ...
        local indicator = {
            ...
            position = data.position or UDim2.new(0,15,0,300),
            values = {},
        ...
        function indicator:Update()
            ...
            for _,v in next, self.values do
                ...
                v.objects.valueLabel.Position = UDim2.new(1,-(v.objects.valueLabel.TextBounds.X + 3),0,0)
                v.objects.background.Position = UDim2.new(0,0,1,3 + yPos)
            ...
            self.objects.background.Size = UDim2.new(0,xSize + 8,0,16)
        ...
        function indicator:AddValue(data)
            ...
            self.keyIndicator = self.NewIndicator({title = 'Keybinds', pos = UDim2.new(0,15,0,325), enabled = false});
            self:SetOpen(true);
            self.hasInit = true    
        end
...
function library:CreateSettingsTab(menu)
    ...
    local configSection = settingsTab:AddSection('Config',"Right")
    local mainSection = settingsTab:AddSection('Keybinds', "Left")

    mainSection:Toggle({Name = "Toggle", def = false, Pointer = "keybind_indicator", Callback = function(bool)
        library.keyIndicator:SetEnabled(bool);
     end})

     mainSection:Slider({name = "Position X", min = 0, max = 100, def = 1, decimals = 0.01, Pointer = "keybind_indicator_x", Callback = function()
        local x, y
        ...
        library.keyIndicator:SetPosition(UDim2.new(0, x, 0, y));
    end})
